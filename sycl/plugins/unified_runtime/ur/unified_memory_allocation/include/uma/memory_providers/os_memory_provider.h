...
#ifdef __linux__
enum uma_mem_visibility {
  VisibilityShared = MAP_SHARED,
  VisibilityPrivate = MAP_PRIVATE
};
#else
enum uma_mem_visibility { VisibilityShared, VisibilityPrivate };
#endif
...
enum uma_numa_flags { NumaFlagsStrict = 1, NumaFlagsMove, NumaFlagsMoveAll };
#endif

typedef struct uma_os_memory_provider_config_t
    *uma_os_memory_provider_config_handle_t;

uma_os_memory_provider_config_handle_t umaOsMemoryProviderConfigCreate();
void umaOsMemoryProviderConfigDestroy(uma_os_memory_provider_config_handle_t);

void umaOsMemoryProviderConfigSetMemoryProtection(
    uma_os_memory_provider_config_handle_t handle, int memoryProtection);
...
enum uma_result_t
umaOsMemoryProviderCreate(uma_os_memory_provider_config_handle_t,
                          uma_memory_provider_handle_t *);
void umaOSMemoryProviderDestroy(uma_memory_provider_handle_t);
