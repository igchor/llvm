...
struct urEventWaitTest : uur::urDeviceTest {
  void SetUp() override {
    ...
    for (size_t i = 0; i < maxNumContexts; ++i) {
      ...
      ASSERT_SUCCESS(urEnqueueMemBufferWrite(queue, src_buffer[i], true, 0, size,
                  input[i].data(), 0, nullptr, nullptr));
    }
  }
...
TEST_P(urEventWaitTest, Success) {
  ...
  ASSERT_SUCCESS(urEnqueueMemBufferCopy(queues[0], src_buffer[0], dst_buffer[0], 0, 0,
                                        size, 0, nullptr, &event1));
  std::vector<uint32_t> output(count, 1);
  ur_event_handle_t event2 = nullptr;
  ASSERT_SUCCESS(urEnqueueMemBufferRead(queues[0], dst_buffer[0], false, 0, size,
...
TEST_P(urEventWaitTest, WithMultipleContexts) {
  ...
  for (size_t i = 0; i < maxNumContexts - 1; i++) {
    ...
    ur_event_handle_t event = nullptr;
    ASSERT_SUCCESS(urEnqueueMemBufferCopy(queues[i], src_buffer[i], src_buffer[i + 1], 0, 0,
                                          size, waitEvent ? 1 : 0, waitEvent, &event));
                                          events.push_back(event);
  }

  ur_event_handle_t event1 = nullptr;
  ASSERT_SUCCESS(urEnqueueMemBufferCopy(queues.back(), src_buffer.back(), dst_buffer.back(), 0, 0,
    size, 1, &events.back(), &event1));

  ur_event_handle_t event2 = nullptr;
  ASSERT_SUCCESS(urEnqueueMemBufferRead(queues.back(), dst_buffer.back(), false, 0, size,
                                        output.data(), 0, nullptr, &event2));
