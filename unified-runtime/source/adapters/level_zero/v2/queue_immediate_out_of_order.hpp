...
template <size_t N> struct signal_events_handle {
  std::array<ur_event_handle_t, N> events;
  std::array<bool, N> isExternal;

  signal_events_handle() {
  ...
  ~signal_events_handle() {
    for (size_t i = 0; i < N; ++i) {
      if (isExternal[i] && events[i] != nullptr) {
        events[i]->release();
      }
    }
  }

  void assign(size_t index, ur_event_handle_t event, bool external) {
    if (isExternal[index] && events[index] != nullptr) {
      events[index]->release();
    }

    events[index] = event;
    isExternal[index] = external;
...
private:
  ...
  std::array<ur_event_handle_t, numCommandLists> internalSignalEvents;
  ...
  signal_events_handle<numCommandLists> signalEvents;
  ...
  // Create a new event if requested bu the user, otherwise return
  // internal signal event associated with a give command list.
  // Retain controls whether to call retain() on the event
  // (only for newly created events).
  template <bool Retain>
  ur_event_handle_t createOrUseInternalSignalEvent(uint32_t cmdListId,
...
public:
  ...
  ur_result_t enqueueKernelLaunch(
    ...
    return commandListManagers.lock()[commandListId].appendKernelLaunch(
        ...
        phEventWaitList,
        createOrUseInternalSignalEvent<false>(commandListId, phEvent));
  }
  ur_result_t
  ...
  ur_result_t enqueueUSMDeviceAllocExp(
    ...
    return commandListManagers.lock()[commandListId].appendUSMAllocHelper(
        ...
        ppMem, createOrUseInternalSignalEvent<false>(commandListId, phEvent),
        UR_USM_TYPE_DEVICE);
  }
  ...
  ur_result_t enqueueUSMFreeExp(ur_usm_pool_handle_t pPool, void *pMem,
    ...
    return commandListManagers.lock()[commandListId].appendUSMFreeExp(
        ...
        createOrUseInternalSignalEvent<true>(commandListId, phEvent));
  }
