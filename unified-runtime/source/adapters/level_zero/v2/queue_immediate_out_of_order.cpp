...
// constructor.
template <size_t... Is>
std::array<ur_command_list_manager, sizeof...(Is)>
...
void ur_queue_immediate_out_of_order_t::initializeSignalEvents() {
  ...
  for (size_t i = 0; i < numCommandLists; ++i) {
    ...
    (*commandListManagers.get_no_lock())[i].appendUSMFill(
        tmpMem, sizeof(tmpPattern), &tmpPattern, sizeof(tmpPattern), 0, nullptr,
        internalSignalEvents[i]);
    ZE2UR_CALL_THROWS(
        zeCommandListHostSynchronize,
        ((*commandListManagers.get_no_lock())[i].getZeCommandList(), UINT64_MAX));
...
ur_result_t ur_queue_immediate_out_of_order_t::queueFinish() {
  ...
  UR_CALL(commandListManagersLocked[lastCommandListId].appendEventsWait(
      numCommandLists, signalEvents.events.data(), nullptr));
  ZE2UR_CALL(zeCommandListHostSynchronize,
             (commandListManagersLocked[lastCommandListId].getZeCommandList(),
              UINT64_MAX));

  // for (int i = 0; i < numCommandLists; i++) {
  //   ZE2UR_CALL(zeCommandListHostSynchronize, (commandListManagers[i].lock()->getZeCommandList(),
  //                                          UINT64_MAX));
  // }
...
ur_result_t ur_queue_immediate_out_of_order_t::enqueueEventsWaitWithBarrier(
  ...
  if ((flags & UR_QUEUE_FLAG_PROFILING_ENABLE) != 0) {
    for (size_t id = 0; id < numCommandLists; id++) {
      UR_CALL(commandListManagersLocked[id].appendEventsWaitWithBarrier(
