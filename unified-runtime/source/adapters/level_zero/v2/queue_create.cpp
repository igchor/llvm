...
ur_result_t urQueueCreate(ur_context_handle_t hContext,
  ...
  if ((flags & UR_QUEUE_FLAG_OUT_OF_ORDER_EXEC_MODE_ENABLE) != 0 && !zeIndex.has_value()) {
    *phQueue =
      ur_queue_handle_t_::create<lockable<v2::ur_queue_immediate_out_of_order_t>>(
          hContext, hDevice, v2::getZeOrdinal(hDevice), v2::getZePriority(flags), v2::eventFlagsFromQueueFlags(flags), flags);
  } else {
    *phQueue =
      ur_queue_handle_t_::create<lockable<v2::ur_queue_immediate_in_order_t>>(
          hContext, hDevice, v2::getZeOrdinal(hDevice), v2::getZePriority(flags), v2::eventFlagsFromQueueFlags(flags), flags);
  }
...
ur_result_t urQueueCreateWithNativeHandle(
  ...
  *phQueue =
      ur_queue_handle_t_::create<lockable<v2::ur_queue_immediate_in_order_t>>(
          hContext, hDevice, hNativeQueue, v2::eventFlagsFromQueueFlags(flags), flags, ownNativeHandle);
